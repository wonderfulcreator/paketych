...
class CloudproofFF1Engine(Engine):
    ...
    def _transform_letters(self, s: str, mode: str, key: bytes, tweak: bytes) -> str:
        # Collect letter positions (Cyrillic or Latin). We assume cells do not mix scripts.
        positions: List[int] = [i for i, ch in enumerate(s) if ch.isalpha()]
        if not positions:
            return s

        has_ru = any(is_cyrillic_char(s[i]) for i in positions)
        has_en = any((not is_cyrillic_char(s[i])) for i in positions)
        if has_ru and has_en:
            # User said this should not happen; fail loudly.
            raise ValueError(f"Mixed Cyrillic+Latin letters in one cell: {s!r}")

        alphabet_obj = self._a_ru if has_ru else self._a_en
        label = "letters|ru" if has_ru else "letters|en"
        k = self._subkey(key, label)
        tw = self._subtweak(tweak, label)

        original_letters = "".join(s[i] for i in positions)
        letters_lower = original_letters.lower()

        out_lower = self._try_ff1(alphabet_obj, mode, k, tw, letters_lower, what=label)

        out_letters = apply_case_mask(out_lower, original_letters)

        # Reinsert
        buf = list(s)
        for idx, pos in enumerate(positions):
            buf[pos] = out_letters[idx]
        return "".join(buf)
