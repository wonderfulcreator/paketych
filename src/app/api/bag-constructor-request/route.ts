...
// POST /api/constructor-request { size, color, hasLogo }
...
export async function POST(req: NextRequest) {
  try {
    const { size, color, hasLogo } = await req.json();
    ...
    if (resend) {
      ...
      console.log("[constructor-request] RESEND_API_KEY not set, skipping email", { size, color, hasLogo, contact });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[constructor-request POST]", err);
