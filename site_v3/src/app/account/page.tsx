...
function AccountInner() {
  ...
  useEffect(() => {
    if (ready && user) loadOrders();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ready, user]);

  // При переходе с ?refresh=1 (после оформления заказа) — принудительно перезагружаем
  useEffect(() => {
    if (sp.get("refresh") === "1" && user) {
      loadOrders();
  ...
  // Обновляем при возврате на вкладку (например, после оформления заказа)
  useEffect(() => {
    const onFocus = () => { if (user) loadOrders(); };
